...
- **Partial and proposed types.** T2 is partially observed (fixed-yaw evidence, no handover benchmark yet); T4 is now measured (§5.5) but on small per-position samples; T3b, T5, and T6 remain proposed with protocols, not yet measured. We label each accordingly and do not claim otherwise.
