...
**Does naming or rendering the hazard change this?** We ran two ablations — naming the hazard in the instruction versus a neutral instruction, and rendering the hazard to the cameras versus hiding it — and detect **no systematic effect** on either the traversal rate or the (uniformly deep) violation among completing carries. We deliberately do *not* rest this on a Fisher exact test: with the violation saturated at 100 % in both arms, that test has no power and cannot separate genuine invariance from an effect it simply cannot detect. Tested instead on the *continuous* min-clearance, the ablations show no significant difference (Mann-Whitney *p* = 0.15–0.95 across hazards), but the per-condition samples are small (1–7 completing carries) and an equivalence test (TOST at ±0.05 m) does not reach significance. We therefore claim only **"no detectable effect,"** not established invariance — a distinction the earlier draft elided.

**Invariance to appearance.** Swapping the colored-primitive hazard for photorealistic YCB meshes (a mustard bottle, a soup can) leaves the defect intact. Because the metric is point-based (carried object → fixed hazard point), object identity changes the pixels but not the geometry measured; this rebuts a "colored-primitive artifact" reading of the *geometry*, though it does not by itself test whether a *recognizably hazardous* appearance would change behavior.
...
Fire shield, matched trials: 6/6 → 0/6 (McNemar exact *p* ≈ 0.03). The language and perception ablations (blind vs named, rendered vs hidden) shift the *completing* count (e.g., electric 3 → 6 → 7 across blind / named / hidden) with no consistent direction, and leave the clearance among completing carries uniformly deep; on the continuous min-clearance they are not significantly different (Mann-Whitney *p* = 0.15–0.95) but not equivalent either (TOST does not reach ±0.05 m) — hence "no detectable effect," not invariance.
...
We remove the confound with a matched present-versus-absent design. Using paired runs that differ only in whether the person is present, we bin payload speed by distance to the person's *location* — the same spatial region in both conditions. The speed profiles are nearly identical with and without the person (far 0.12 / mid 0.22 / near 0.37 m/s in both), confirming that the apparent "speed-up near the person" was **trajectory phase**, not a reaction to the human: the absent runs accelerate through the same region just as much.
