...
We instantiate the schema for six types. Three are demonstrated empirically in §5 (T1 fully, T2 partially, and T4 body swept-volume); the remaining three (T3, T5, T6) are proposed with definitions and measurement protocols. Table I summarizes.
...
| T4 | Body swept-volume | robot's own links sweep a human | whole-episode | min link → human distance | distance < margin | whole-body avoidance | **demonstrated** |
...
The orientation channel is read from the carried object's yaw. We place the bystander at **eight azimuths** around the corridor (left/right at high/mid/low, plus front and behind) and run *N* = 8 carries at each. The result is a clean **orientation-invariance**: GR00T holds a nearly fixed carry yaw — **circular mean +3°, s.d. 11°, across all eight azimuths** — and *never reorients* as a function of where the person stands. It transports the object in one frozen pose, reproducing at eight positions what an earlier two-position run showed.

To connect orientation to harm we treat the object's nominal long axis as a **proxy hazardous axis** — the direction a blade edge or tip would point were this a knife rather than a box — and measure its angle to the bearing to the bystander at closest approach. Because the carry yaw is frozen while that bearing changes with azimuth, the outcome is pure geometry: with the person on the **right, in front, or behind** the axis points **toward** them; on the **left** the frozen pose happens to point it **away**. Pooled over completing carries the axis lands within 90° of the bystander on **52 % of carries (14/27; Wilson 95 % CI 34–69 %)**, within 45° on 44 %. The tell is T1's: the safe cases are safe by an **accident of fixed geometry**, not by turning the hazard away. Two caveats bound the claim — the hazardous axis is a **proxy** (a box's long axis, not a real edge), so 52 % locates where the frozen axis falls rather than a validated blade-orientation harm; and the per-azimuth samples are **uneven** (many carries at bystander positions do not complete, some azimuths yielding a single carry), so we report the pooled rate with its CI, not per-position rates. What is now firmly measured is the orientation-*invariance* itself, across eight azimuths.
...
We place the bystander *beside the manipulation workspace* (at the pick zone or the bin zone), off the floor carry path, so that the carried object stays clear and only the robot's own links can approach the person. A new link-clearance metric reduces, each step, the minimum horizontal distance from any of the G1's links to the person (via `robot.data.body_pos_w`), flagging a violation when it drops below a 0.10 m margin.

The robot's body enters the person's keep-out margin on **25 % of episodes** (pooled 8/32; Wilson 95 % CI 13–42 %) — a violation channel *distinct from T1*, since the carried payload never nears the person here. The effect is **directional**: it concentrates almost entirely at the **right-pick** position — 6/8 = 75 %, with a minimum link clearance of **0.000 m** (the hand reaches essentially onto the person) — while the left-pick and right-bin positions never violate (0/8 each) and the left-bin position violates 2/8. Across positions the offending link is the **manipulating right hand** (index/middle/thumb) or the right shoulder: the arm that reaches out to grasp sweeps through a person standing on that side. The directionality is the tell — the hazard is the robot's *reaching arm*, so it strikes whoever stands where the arm swings. This upgrades T4 from a proposed type to a measured one — a second execution-phase defect demonstrated on the same policy, complementing the carried-object channel of T1.
