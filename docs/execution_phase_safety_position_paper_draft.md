...
*Position paper — draft v0.9 · 2026-09-02*
...
3. **An empirical spine — two defect channels, plus a measured orientation-invariance and a de-confounded null (§5).** On GR00T N1.7 driving a Unitree G1 in Isaac Sim, every carry that completes the traversal passes through the hazard (T1: 10/10; the unconditioned rate is 29 %, the failures being early non-traversals rather than detours), and naming or rendering the hazard produces no detectable change (on an underpowered ablation); separately, the robot's own arm enters a bystander's keep-out margin on 25 % of episodes (T4, body swept-volume — the body, not the payload; 75 % at the right-pick position); across eight bystander azimuths the carried object's orientation is held fixed and never reoriented (T2), aiming its nominal hazardous axis (a proxy, not a real edge) at the person on 52 % of completing carries; and a de-confounded, episode-level T3a analysis finds no reliable modulation of speed by a person's presence.
...
We are explicit throughout about what is *demonstrated* versus *proposed*: three of the six types carry measurements (T1 fully; T4, body swept-volume; and T2 partially, on a proxy hazardous axis); the other three carry definitions and measurement protocols. This is a taxonomy grounded in data where we have it and honest about where we do not.
...
**What is, and is not, new here.** Constraining *how* a motion unfolds is not new: potential fields [8], control barrier functions [9], safe-RL shielding [10], and ISO/TS 15066 [11] all do it; safe learning for *learned* controllers is itself a mature field [16]; and a growing line of work wraps learned visual policies in runtime safety monitors and latent-space safety filters that veto unsafe actions during execution [17], [18]. We do not claim to invent execution-phase constraints. Safety for VLAs specifically has, moreover, begun to be addressed directly: SafeVLA [21] casts deployment-time physical harm — to the environment, the robot, and humans — as a constrained MDP and reduces it via safe RL, and VLSA [22] attaches a plug-and-play, control-barrier safety layer to a pretrained VLA policy. Our claim is therefore narrower and, we think, complementary: (i) this axis is **not yet decomposed into measurable, per-type, trajectory-level predicates paired with fixability ablations** — success-centric benchmarks and instruction/outcome safety still leave it largely unmeasured type-by-type for foundation-model policies, and the safe-RL and safety-layer works above are *remedies* rather than a diagnostic account of what breaks and what kind of fix each failure needs; (ii) we give it a **measurable taxonomy** that turns scattered concerns into a family of ⟨channel, phase, quantity, predicate, metric, fixability⟩ tuples for a carried-hazard, humanoid-VLA setting; and (iii) we provide a **defect demonstration** on a state-of-the-art VLA, with fixability ablations that locate each failure on the prompting / perception / architecture spectrum. The classical machinery above is precisely the space of *remedies* our fixability axis points to; the contribution is the measurement frame and the empirical finding, not the control theory. Concurrent VLA-safety benchmarks are appearing — HazardArena [19] reports that VLAs trained on safe scenes act unsafely in matched "unsafe twins"; ForesightSafety-VLA [23] proposes a 13-category diagnostic taxonomy spanning physical-interaction, language, and perception safety; and LIBERO-Safety [24] procedurally generates physical- and semantic-safety scenarios for VLAs. These organize safety by *scenario and input modality*; our axis instead partitions by *what is judged* — a predicate on the trajectory, irreducible to predicates on the instruction or the terminal state — and instantiates each type as a success-conditioned ⟨channel, phase, quantity, predicate, metric, fixability⟩ tuple carrying prompting/perception/architecture fixability ablations, demonstrated on a carried-hazard-vs-person clearance defect. The distinction we draw is the organizing dimension, and we claim a reframing, not priority: not the discovery that trajectories can be unsafe.
...
We instantiate the schema for six types. Three are demonstrated empirically in §5 (T1 fully, T2 partially, and T4 body swept-volume); the remaining three carry definitions and measurement protocols (T3 and T5 proposed, T6 planned). Table I summarizes.
...
None of these undercuts the position: two measured defect channels — an appearance-robust path/keep-out defect (T1) and a body-swept-volume defect (T4, the robot's own arm) — plus a de-confounded null on speed-and-separation (T3a) are enough to show that execution-phase harm is real, systematic, and not addressed by the existing two axes.
