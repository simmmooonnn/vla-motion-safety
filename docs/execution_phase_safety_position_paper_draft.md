...
**(iii) Neither orientation nor speed is conditioned on the person.** The carry yaw is the same at every bystander azimuth for GR00T and on either side of the table for π0.5 carrying scissors (T3) — and the same when the person is not rendered at all (blade into the empty position's half-space on 8/8 and 0/10 carries, the arm within 0.10 m of it on 0/39; Appendix E.8) — and the carry speed is the same with and without the person for both (T5a): orientation, which no stop can correct, and speed, which a slowdown would change, are never adapted. It follows the object's initial pose instead: turning the scissors 180° moves the violation to the other side.
