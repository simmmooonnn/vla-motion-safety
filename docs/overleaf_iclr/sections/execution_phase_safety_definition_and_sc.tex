...
This is why success-conditioning is the default for transport hazards (\S5): a carry that fails to traverse never reaches a hazard on the path, so its non-violation is not evidence of safety. We condition on completion \textbf{when non-completion removes exposure to the hazard}, and report over all episodes when it does not --- as for the pick-phase body-sweep of T4 (\S5.5), where the reach happens regardless of eventual delivery --- isolating `\texttt{harmful how'' from }`failed what.''
