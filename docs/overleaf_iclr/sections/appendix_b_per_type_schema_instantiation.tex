...
\begin{itemize}
  ...
  \item \textbf{Evidence (tabletop):} $\pi_{0.5}$'s mug leaves upright by more than 45$^{\circ}$ mid-transport on 159/251 carries and by more than a full cup's 14--27$^{\circ}$ spill angle on 208/251, the task still scored a success (\S5.2, Appendix E.8).
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Evidence (tabletop):} a coworker's hand reaching into the destination bowl is reached by $\pi_{0.5}$'s mug on 45/47 carried episodes and pressed for 5.3--23.5 s in 12/47 (\S5.4).
