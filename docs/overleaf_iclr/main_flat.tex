...
\begin{table}[t]
\centering\scriptsize\setlength{\tabcolsep}{3pt}
\caption{Main results: one score per policy and dimension. Bold: mean of the dimension's fixed sub-type set (\S4.2), formed when every member has $\geq$ 8 scored episodes; brackets: sub-type rates (counts below eight). T3 pooled over bearings (chance 50 \%); speed and force is {T5a, T5b} on the G1 (blank: T5a has six episodes, below the floor) and {T5b} on the tabletop; T5c and the tabletop T5a are exposure, in Table IIIb. Last row: a scripted straight-line carry from privileged state, blind to the person (\S5.5) --- a column on which it scores like the policies is set by scene or task. \emph{Witness}: a compliant completion shown in the scene.}
\label{tab:III}
\vspace{4pt}
...
scripted straight-line carry \textperiodcentered{} Franka (control) & \textbf{51} (T1 100, T2 1) & \textbf{27} (T3 47, T4 8) & \textbf{6} (T5b 6) & --- (T6 100, T6b 0/1) \\
...
\textbf{A scripted straight-line carry as the control.} A scripted carrier reads the poses from the simulator, ignores the person and carries on a straight line (414 carries; last row of Table~\ref{tab:III}). Its arm is inverse-kinematics-driven, but its payload is held kinematically rather than grasped, so it witnesses geometry only: the cells it shares with $\pi_{0.5}$ agree on the body sweep and do not separate on orientation, its tilt and contact cells are properties of the attachment, and its off-path keep-out result is the trajectory witness of \S5.1 (Appendix E.8).
...
\begin{itemize}
  ...
  \item \textbf{Evidence (tabletop):} $\pi_{0.5}$'s mug leaves upright by more than 45$^{\circ}$ mid-transport on 211/298 carries and by more than a full cup's 14--27$^{\circ}$ spill angle on 1010/1208, the task still scored a success (\S5.2, Appendix E.8).
\end{itemize}
...
scripted straight-line carry \textperiodcentered{} Franka (control) & 24/24 = 100 \% [86, 100] & 3/232 = 1 \% [0, 4] & 90/192 = 47 \% [40, 54] & 14/177 = 8 \% [5, 13] & (232/232 exposure) & 1/16 = 6 \% [1, 28] & 16/16 = 100 \% [81, 100] & 0/1 (below the floor) \\
\bottomrule
\end{tabular}
\end{table}
...
\label{tab:IIIc}
\vspace{4pt}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.273\linewidth}>{\raggedright\arraybackslash}p{0.105\linewidth}>{\raggedright\arraybackslash}p{0.105\linewidth}>{\raggedright\arraybackslash}p{0.095\linewidth}>{\raggedright\arraybackslash}p{0.114\linewidth}>{\raggedright\arraybackslash}p{0.214\linewidth}@{}}
\toprule
Quantity & GR00T N1.6 \textperiodcentered{} G1 & $\pi_{0.5}$ \textperiodcentered{} Franka & $\pi_0$ \textperiodcentered{} Franka & GR00T N1.6-DROID \textperiodcentered{} Franka & scripted straight-line carry \textperiodcentered{} Franka (control) \\
\midrule
...
T4 above the 14--27$^{\circ}$ spill angle (27$^{\circ}$) & 0/17 & 260/298 = 87 \% [83, 91] & 40/52 = 77 \% [64, 86] & 23/27 = 85 \% [68, 94] & 21/177 = 12 \% [8, 17] \\
...
\begin{table}[H]
\centering\scriptsize\setlength{\tabcolsep}{3pt}
\caption{Coverage: attempted / carried / delivered episodes per work surface and policy (every tabletop cell; probes and demos excluded; 4341 episodes, 3110 carried, 2067 delivered).}
\label{tab:IVb}
\vspace{4pt}
