%% ICLR 2027 submission — seed generated from the markdown draft (v0.32). SINGLE-FILE version: all sections inlined; only figures/ and refs.bib are external.
...
T6 & Dynamic reactivity & human/hazard moves; policy fails to react & min person separation; time-to-collision & contact with the crossing person (separation at contact distance; TTC reported) & \textbf{contact} 11/11; stop layer 0/8 \\
\bottomrule
\end{tabular}
\end{table}
...
T3a speed vs person & 6 completing, person present & 0/6 slow near the person; 6/6 inside $d_0$ & --- & stop 0.45 m: fires 6/6, completes 0/6 & --- \\
...
T6 crossing person & 11 completing, 3 seeds & contact distance 11/11 [74, 100]; stall 6/11; person-absent control 0/3 & 30 \% $\rightarrow$ 25 \% (p = 1.0) & repulsion 0.50--0.80 m: 12/17 at contact; stop 0.50 m: 0/8, completion kept & --- \\
\bottomrule
\end{tabular}
\end{table}
...
Two further items belong in every type's protocol: a \textbf{feasibility witness} --- a scripted or teleoperated trajectory that completes the scene without violating the predicate, so that a violation rate is attributable to the policy rather than to the scene --- and a \textbf{protective-stop reference layer}, the reactive response the standards prescribe, reported next to the repulsion shield --- run here for T6 and T3a (\S5.4, \S5.7). \textbf{Release.} Scene configurations, metric recorders, analysis scripts and every per-episode log behind Appendix A are released (anonymized repository with the submission); adding a policy is a server swap.
...
We state the boundaries plainly; Appendix F expands each. The evidence is \textbf{simulation-only}, mostly one policy on one embodiment, with \textbf{small cells} (T1 headline cells 3--8 completing carries, T3a \emph{n} = 6, T4 \emph{N} = 8 per position, T6 \emph{n} = 11 plus 12-episode controls) and a person cell of 21 completing carries whose payload is labelled, not physically, hazardous. Two measured rates --- T3a's envelope count and T4 --- are partly set by the scene or by the scoring geometry and are labelled \textbf{attribution-pending} until a violation-free reference trajectory exists (the stopped T6 carry provides one for T6); what is policy-attributable today is the absence of any slowing, detour or stop before contact. The ablations are \textbf{ceiling-limited} and the paired probe is, for T1 and T6, a completion comparison (\S6). The shield is \textbf{oracle-fed and margin-tuned} per hazard. The static bystander proxies have \textbf{no collider}; the crossing one records no force, and its 0.3--1.2 m/s sweep rests on six carried encounters. Keep-out radii are \textbf{illustrative, not standards-derived}: a defensible separation under ISO/TS 15066 \citep{ref11} / ISO 13855 \citep{ref20} would be several times larger, which only deepens the violation (Appendix D). None of this undercuts the case: a keep-out defect on two policies, a body-penetration defect, a contact defect under a crossing person and a standards-referenced absence of slowing show that execution-phase harm is real, systematic and unaddressed by the existing two axes.
...
\label{tab:V}
\vspace{4pt}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.270\linewidth}>{\raggedright\arraybackslash}p{0.066\linewidth}>{\raggedright\arraybackslash}p{0.073\linewidth}>{\raggedright\arraybackslash}p{0.160\linewidth}>{\raggedright\arraybackslash}p{0.226\linewidth}>{\raggedright\arraybackslash}p{0.109\linewidth}@{}}
\toprule
...
\label{tab:X}
\vspace{4pt}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.286\linewidth}>{\raggedright\arraybackslash}p{0.060\linewidth}>{\raggedright\arraybackslash}p{0.067\linewidth}>{\raggedright\arraybackslash}p{0.146\linewidth}>{\raggedright\arraybackslash}p{0.206\linewidth}>{\raggedright\arraybackslash}p{0.140\linewidth}@{}}
\toprule
Channel / cell & attempted & completing & violating / completing & violation rate (Wilson 95 \% CI) & completing rate \\
\midrule
...
+ protective stop, 0.50 m (SSM at 0.06 m/s) & 12 & 6 & 0 / 8 carried (minimum 0.39 m); fires 11 / 12, overshoot 0.06 m & 0 \% [0, 32] & 50 \% \\
+ protective stop, 0.94 m (ISO 13855 walking speed) & 12 & 0 & 0 / 4 carried; fires 11 / 12, never completes & --- & 0 \% \\
robot-triggered crossing, 0.3 / 0.6 / 1.2 m/s & 36 & 0 & 5 / 6 carried struck by the person (box knocked out) & --- & 0 \% (carried 6 / 36) \\
...
\subsection{T3 \textperiodcentered{} Contact force \& speed limiting --- \emph{T3\textnormal{a}: defect (no SSM); T3\textnormal{b} proposed}}
...
\begin{itemize}
  ...
  \item \textbf{Fixability:} a speed/force governor keyed to human proximity. (T3a is scored against the ISO/TS 15066 speed-and-separation envelope in \S5.4 --- 6/6 completing carries pass the person at full speed inside the stop distance; T3b power-and-force limiting requires a contact sensor and a re-run.)
\end{itemize}
...
\textbf{Controls (2026-09 round; 12 episodes per cell, seed 42, one GR00T server).} \emph{Collider-off twin.} With the capsule's collider disabled the box passes through the person on 7/7 completing carries --- minimum 0.008--0.26 m from the axis --- at 0.33 m/s within 0.60 m of the person versus 0.20 m/s beyond it; the stalls of the collider cell are therefore the body's block, not a learned stop. \emph{Crossing-speed sweep.} At 0.3, 0.6 and 1.2 m/s the person waits until the robot base passes $y = -0.35$ m, crosses, and stands 0.8 m past the path; carried encounters were 2, 3 and 1 of 12 (pick failures --- the hand pushing the box 0.22 m back on the shelf --- ran 30/36 in these three cells against 17/36 in the other T6 cells of the day, a between-cell variance we could not attribute and are re-running on a second seed with a fresh server per cell). In 5/6 carried encounters the person strikes the carried box and knocks it from the grasp (episode ends early); box speed in the last second before contact is 0.36--0.57 m/s against 0.30--0.38 m/s two seconds earlier --- no deceleration; the sixth encounter is passed at 0.69 m. \emph{Larger repulsion margins.} The live-tracking shield at 0.60 and 0.80 m keeps the box 0.32--0.45 m from the axis over six carried episodes (2/6 at contact distance, none beyond 0.45 m): the margin shaves the intrusion and does not remove it. \emph{Protective stop.} With the base command zeroed while the person is within 0.50 m (hysteresis 0.10 m; separation taken to the nearer of the carried object and the base), the stop fires in 11/12 episodes (1--2 firings, median 6.8 s stopped, range 3.3--9.3 s), the separation after the stop command falls a further 0.013--0.112 m (median 0.064 m --- the stopping distance at this speed), no carried episode reaches contact (0/8; minimum 0.39 m) and completion is 6/12, against 11/24 unshielded and 5/12 (two seeds) in the powered re-run. At 0.94 m --- the ISO 13855 distance under its 1.6 m/s human-approach assumption --- the stop fires in 11/12 episodes for a median 14 s (range 12--24 s of a 30 s episode) and no carry completes (0/12; minimum separation 0.59 m): the stop distance trades completion for separation, and the corridor cannot host a 0.94 m separation while the person crosses it. \emph{Static person.} The same layer at 0.45 m on the T1 person cell (\S5.4) fires 6/6 and never releases (0/6 complete), so a stop can serve the dynamic channel and not the static ones.
