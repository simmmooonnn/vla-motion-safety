...
\begin{itemize}
  ...
  \item \textbf{Evidence (tabletop):} $\pi_{0.5}$'s mug leaves upright by more than 45$^{\circ}$ mid-transport on 130/202 carries and by more than a full cup's 14--27$^{\circ}$ spill angle on 691/820, the task still scored a success (\S5.2, Appendix E.8).
\end{itemize}
...
\begin{table}[H]
\centering\small
\caption{Coverage: attempted / carried / delivered episodes per work surface and policy (every tabletop cell; probes and demos excluded; 3244 episodes, 2202 carried, 1387 delivered).}
\label{tab:IVb}
\vspace{4pt}
\begin{tabular}{@{}llll@{}}
\toprule
...
\textbf{T4.} $\pi_{0.5}$ carries a mug tilted in its grasp: over 202 carries in 27 canonical cells at six surfaces its axis leaves upright by more than 45$^{\circ}$ mid-transport on 130 (64 \%) and by more than 27$^{\circ}$ on 167; 118 of the 130 are delivered to the bowl and scored successful (the task battery's tilt rates are in Table~\ref{tab:IV}). Told to keep hot coffee upright, it still tilts the mug past 45$^{\circ}$ on 16/24 (27$^{\circ}$: 21/24): the command does not change the carry. Where the tilt happens matters for a scald: over every $\pi_{0.5}$ mug carry with a person present, the peak tilt exceeds 45$^{\circ}$ within 0.60 m of them on 341/1046 and farther away on 355/1046; the payload leaves the work surface (a drop) on 24/1736 episodes. A pitcher (a taller, heavier liquid vessel) and a cordless drill, a third hazardous object (bit forward), are never lifted by $\pi_{0.5}$ (0/16 and 0/32 attempts): both are capability boundaries, not safety rates. $\pi_0$ tilts less where it carries (39/79 above 45$^{\circ}$).
