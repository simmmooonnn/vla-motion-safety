...
$\pi_{0.5}$ \textperiodcentered{} Franka & \textbf{51} (T1 100, T2 1) & \textbf{54} (T3 43, T4 64) & --- (T5b 7) & \textbf{87} (T6 94, T6b 79) \\
...
\begin{itemize}
  ...
  \item \textbf{Evidence (tabletop):} $\pi_{0.5}$'s mug leaves upright by more than 45$^{\circ}$ mid-transport on 130/202 carries and by more than a full cup's 14--27$^{\circ}$ spill angle on 727/875, the task still scored a success (\S5.2, Appendix E.8).
\end{itemize}
...
$\pi_{0.5}$ \textperiodcentered{} Franka & 24/24 = 100 \% [86, 100] & 3/285 = 1 \% [0, 3] & 32/75 = 43 \% [32, 54] & 130/202 = 64 \% [58, 71] & (304/304 exposure) & 6/83 = 7 \% [3, 15] & 78/83 = 94 \% [87, 97] & 34/43 = 79 \% [65, 89] \\
...
T4 above the 14--27$^{\circ}$ spill angle (27$^{\circ}$) & 0/17 & 167/202 = 83 \% [77, 87] & 40/52 = 77 \% [64, 86] & 23/27 = 85 \% [68, 94] & 18/105 = 17 \% [11, 25] \\
...
\begin{table}[H]
\centering\small
\caption{Coverage: attempted / carried / delivered episodes per work surface and policy (every tabletop cell; probes and demos excluded; 3356 episodes, 2297 carried, 1460 delivered).}
\label{tab:IVb}
\vspace{4pt}
\begin{tabular}{@{}llll@{}}
\toprule
...
\textbf{T4.} $\pi_{0.5}$ carries a mug tilted in its grasp: over 202 carries in 27 canonical cells at six surfaces its axis leaves upright by more than 45$^{\circ}$ mid-transport on 130 (64 \%) and by more than 27$^{\circ}$ on 167; 118 of the 130 are delivered to the bowl and scored successful (the task battery's tilt rates are in Table~\ref{tab:IV}). Told to keep hot coffee upright, it still tilts the mug past 45$^{\circ}$ on 16/24 (27$^{\circ}$: 21/24): the command does not change the carry. Where the tilt happens matters for a scald: over every $\pi_{0.5}$ mug carry with a person present, the peak tilt exceeds 45$^{\circ}$ within 0.60 m of them on 357/1101 and farther away on 357/1101; the payload leaves the work surface (a drop) on 24/1776 episodes. A pitcher (a taller, heavier liquid vessel) and a cordless drill, a third hazardous object (bit forward), are never lifted by $\pi_{0.5}$ (0/16 and 0/32 attempts): both are capability boundaries, not safety rates. $\pi_0$ tilts less where it carries (39/79 above 45$^{\circ}$).

\textbf{T5a.} Every transport with the person at the table passes inside the 0.94 m stop distance (304/304; exposure, not scored --- the arm never leaves it), and the near-band payload speed is 0.109 vs 0.113 m/s, Welch \emph{p} = 0.79, \emph{n} = 26 vs 13 with the person visible or the position empty: no speed-and-separation behavior.
