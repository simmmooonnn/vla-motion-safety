...
$\pi_{0.5}$ \textperiodcentered{} Franka & \textbf{50} (T1 100, T2 1) & \textbf{57} (T3 43, T4 71) & \textbf{7} (T5b 7) & \textbf{83} (T6 94, T6b 72) \\
$\pi_0$ \textperiodcentered{} Franka & \textbf{50} (T1 100, T2 1) & \textbf{46} (T3 42, T4 50) & \textbf{0} (T5b 0) & --- (T6 78, T6b 4/7) \\
GR00T N1.6-DROID \textperiodcentered{} Franka & --- (T1 2/2, T2 8) & \textbf{51} (T3 20, T4 81) & \textbf{0} (T5b 0) & --- (T6 56) \\
scripted straight-line controls \textperiodcentered{} Franka & \textbf{51} (T1 100, T2 1) & \textbf{31} (T3 47, T4 15) & \textbf{6} (T5b 6) & --- (T6 100, T6b 0/1) \\
...
$\pi_0$, dining table: wk\_mug\_s11 (seed 11) & 8 & 4 carried, 2 delivered & T4 1/4 above 45$^{\circ}$ (3 above 27$^{\circ}$); T6 0/4 reach the hand; T5b peak 0 N & --- & 25 \% \\
...
\begin{itemize}
  ...
  \item \textbf{Evidence (tabletop):} $\pi_{0.5}$'s mug leaves upright by more than 45$^{\circ}$ mid-transport on 211/298 carries and by more than a full cup's 14--27$^{\circ}$ spill angle on 1038/1246, the task still scored a success (\S5.2, Appendix E.8).
\end{itemize}
...
$\pi_{0.5}$ \textperiodcentered{} Franka & 56/56 = 100 \% [94, 100] & 3/429 = 1 \% [0, 2] & 32/75 = 43 \% [32, 54] & 211/298 = 71 \% [65, 76] & (475/475 exposure) & 6/83 = 7 \% [3, 15] & 78/83 = 94 \% [87, 97] & 58/80 = 72 \% [62, 81] \\
$\pi_0$ \textperiodcentered{} Franka & 16/16 = 100 \% [81, 100] & 1/162 = 1 \% [0, 3] & 5/12 = 42 \% [19, 68] & 26/52 = 50 \% [37, 63] & (56/56 exposure) & 0/9 = 0 \% [0, 30] & 7/9 = 78 \% [45, 94] & 4/7 (below the floor) \\
...
scripted straight-line controls \textperiodcentered{} Franka & 24/24 = 100 \% [86, 100] & 3/232 = 1 \% [0, 4] & 90/192 = 47 \% [40, 54] & 4/26 = 15 \% [6, 34] & (232/232 exposure) & 1/16 = 6 \% [1, 28] & 16/16 = 100 \% [81, 100] & 0/1 (below the floor) \\
\bottomrule
\end{tabular}
\end{table}
...
T4 above the 14--27$^{\circ}$ spill angle (27$^{\circ}$) & 0/17 & 260/298 = 87 \% [83, 91] & 40/52 = 77 \% [64, 86] & 23/27 = 85 \% [68, 94] & 17/26 = 65 \% [46, 81] \\
...
\begin{table}[H]
\centering\scriptsize\setlength{\tabcolsep}{3pt}
\caption{Coverage: attempted / carried / delivered episodes per work surface and policy (every tabletop cell; probes and demos excluded; 4493 episodes, 3189 carried, 2128 delivered).}
\label{tab:IVb}
\vspace{4pt}
...
\textbf{T4.} $\pi_{0.5}$ carries a mug tilted in its grasp: over 298 carries in 39 canonical cells at six surfaces its axis leaves upright by more than 45$^{\circ}$ mid-transport on 211 (71 \%) and by more than 27$^{\circ}$ on 260; 199 of the 211 are delivered to the bowl and scored successful (the task battery's tilt rates are in Table~\ref{tab:IV}). Told to keep hot coffee upright, it still tilts the mug past 45$^{\circ}$ on 16/24 (27$^{\circ}$: 21/24): the command does not change the carry, nor at the kitchen counter (21/24 past 45$^{\circ}$), the office desk (22/24), the packing station (11/13) or the drawer kitchen (14/14). Where the tilt happens matters for a scald: over every $\pi_{0.5}$ mug carry with a person present, the peak tilt exceeds 45$^{\circ}$ within 0.60 m of them on 548/1477 and farther away on 416/1477; the payload leaves the work surface (a drop) on 29/2400 episodes. A pitcher (a taller, heavier liquid vessel) and a cordless drill, a third hazardous object (bit forward), are never lifted by $\pi_{0.5}$ (0/16 and 0/32 attempts): both are capability boundaries, not safety rates. $\pi_0$ tilts less where it carries (46/96 above 45$^{\circ}$).

\textbf{T5a.} Every transport with the person at the table passes inside the 0.94 m stop distance (475/475; exposure, not scored --- the arm never leaves it), and the near-band payload speed is 0.109 vs 0.113 m/s, Welch \emph{p} = 0.79, \emph{n} = 26 vs 13 with the person visible or the position empty: no speed-and-separation behavior.
