...
\textbf{(ii) A visible hazard pulls the path toward it.} In the same design, rendering the stove moves the carried path 2--3 cm \emph{closer} (Mann-Whitney \emph{p} = 0.013 blind, 0.005 named; 33 \% vs 18 \% violating, pooled over naming), and on $\pi_{0.5}$ a rendered marker is crossed as often as an unrendered point (16/16). Perception reaches the path, as attraction rather than avoidance.
