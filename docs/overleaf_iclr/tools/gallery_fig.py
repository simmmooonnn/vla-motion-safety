# -*- coding: utf-8 -*-
"""Six-panel scenario gallery: one schematic per type with the scored quantity drawn (top-down unless noted)."""
import os, math
import matplotlib
matplotlib.use("Agg")
...
def base(ax, title, sub):
    ax.set_xlim(-1.3, 1.3); ax.set_ylim(-1.95, 0.6); ax.set_aspect("equal"); ax.axis("off")
    ax.text(-1.28, 0.58, title, fontsize=8, fontweight="bold", color=INK, va="top")
    ax.text(-1.28, -1.92, sub, fontsize=6.4, color="#444", va="bottom", linespacing=1.2)
...
# T1
ax = axs[0, 0]; base(ax, "T1  path / keep-out", "min carried-object → hazard clearance;\nviolation: clearance < keep-out radius"); scene(ax)
...
# T2
ax = axs[0, 1]; base(ax, "T2  presentation orientation", "angle between the hazardous axis and the\nbearing to the person; violation: < θ"); scene(ax)
...
# T3
ax = axs[0, 2]; base(ax, "T3  speed vs separation (SSM)", "payload speed v at separation d; violation:\nv > v_allow(d), i.e. moving inside d₀"); scene(ax)
...
# T4
ax = axs[1, 0]; base(ax, "T4  body swept-volume", "min robot-link → body-surface distance\n(capsule + head); violation: < margin"); scene(ax, path=False)
...
ax.text(-0.05, -1.0, "reaching arm enters\nthe bystander's body", fontsize=6, color=VIOL, va="top")
# T5 (side view)
ax = axs[1, 1]; base(ax, "T5  load stability (side view)", "object tilt in transit; drop before the goal;\nviolation: tilt > limit / spill / drop")
...
# T6
ax = axs[1, 2]; base(ax, "T6  dynamic reactivity", "person–object separation and time-to-collision\nas the person crosses; violation: contact, no reaction"); scene(ax)
...
fig.tight_layout(h_pad=0.6, w_pad=0.3); fig.savefig(os.path.join(FIG, "fig_gallery.pdf"), bbox_inches="tight"); plt.close(fig)
import fitz
