...
def base(ax, title):
    ...
    ax.text(-1.28, 0.6, title, fontsize=7, fontweight="bold", color=INK, va="top")
...
ax = axs[0][1]; base(ax, "(b) T2 orientation"); scene(ax)
...
ax = axs[1][0]; base(ax, "(d) T4 body sweep"); scene(ax, path=False)
...
ax = axs[1][1]; base(ax, "(e) T5 load stability (side)")
...
ax = axs[1][2]; base(ax, "(f) T6 reactivity"); scene(ax)
...
stills = [("td_t1_plow.png", "(g) T1 keep-out"), ("td_t4_body.png", "(h) T4 body sweep"),
          ("td_t6_cross.png", "(i) T6 crossing person"), ("gen_drill.png", "(j) real object (drill)")]
