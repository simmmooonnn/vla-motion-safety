...
def box(x, y, w, h, title, sub, fc, ec, bold=False):
    ...
    ax.text(x + w / 2, y + h - 0.55, title, ha="center", va="center", fontsize=8.2, fontweight="bold" if bold else "normal", color=INK)
    ax.text(x + w / 2, y + 0.55, sub, ha="center", va="center", fontsize=6.6, color="#444")
...
ax.text(5.0, 6.85, "instruction ──── trajectory τ = (s₀, a₀, …, s_T) ──── end state", ha="center", va="center", fontsize=6.4, color="#555")
ax.text(5.0, 4.85, "T1 path / keep-out  ·  T2 presentation orientation  ·  T3 force & speed\nT4 body swept-volume  ·  T5 load stability  ·  T6 dynamic reactivity", ha="center", va="center", fontsize=6.0, color=INK)
ax.add_patch(FancyBboxPatch((0.3, 0.4), 9.4, 2.6, boxstyle="round,pad=0.02,rounding_size=0.25", fc="#f6f6f6", ec=NEUT, lw=0.8))
ax.text(5.0, 2.45, "type = ⟨harm channel, task phase, measured quantity,", ha="center", fontsize=6.2, color=INK)
ax.text(5.0, 1.85, "violation predicate, metric with CI, fixability class⟩", ha="center", fontsize=6.2, color=INK)
ax.text(5.0, 1.05, "fixability: prompt it? · perceive it? · architecture? · external layer?", ha="center", fontsize=6.2, color=VIOL)
...
bx.add_patch(Rectangle((-0.1, 0.25), 1.3, 0.3, fc="#d9c7a3", ec="#8a7350", lw=0.8)); bx.text(0.55, 0.62, "shelf (pick)", ha="center", fontsize=6.5, color="#5a4a2a")
bx.add_patch(Rectangle((-0.5, -1.95), 0.5, 0.45, fc="#c9d4e6", ec=AXIS, lw=0.8)); bx.text(-0.25, -2.15, "bin (place)", ha="center", fontsize=6.5, color=AXIS)
# corridor path
...
bx.text(0.33, -0.28, "carry\n≈1.9 m", fontsize=6.2, color=VIOL, ha="center")
...
bx.text(0.32, -1.02, "T1 hazard on the path\n(strip / stove / person)", fontsize=6.2, color=VIOL, va="center")
...
bx.text(-1.35, -0.32, "T2 hazardous axis\npoints where?", fontsize=6.2, color=GOLD, va="center")
bx.text(-1.35, -0.62, "T3 speed vs\nseparation (SSM)", fontsize=6.2, color=INK, va="center")
bx.text(-1.35, -0.95, "T5 load tilt\n/ spill", fontsize=6.2, color=INK, va="center")
...
bx.text(1.4, -0.4, "T4 bystander\nbeside the\nworkspace:\nthe robot's\nown arm", fontsize=6.2, color=AXIS, va="top", ha="center")
# T6 crossing person
...
bx.text(-1.35, -1.42, "T6 person crossing\nthe corridor (0.06 m/s)", fontsize=6.2, color=AXIS, va="center")
# robot glyph
bx.add_patch(Circle((0.55, 0.05), 0.11, fc="#ddd", ec=INK, lw=0.8)); bx.text(-0.15, 0.05, "G1 + GR00T\n(or Franka + π0.5)", fontsize=6.2, color=INK, va="center", ha="right")
fig.savefig(os.path.join(FIG, "fig_overview.pdf")); plt.close(fig)
import fitz
