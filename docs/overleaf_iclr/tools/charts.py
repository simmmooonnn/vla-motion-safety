...
val  = [100, 100, 100, 25, 52, 0]
col  = [VIOL, VIOL, VIOL, VIOL, VIOL, SAFE]
lab  = ["100% (10/10)", "6/6 outside SSM envelope", "11/11 reach contact", "25% pooled; 100% worst", "52% axis toward person; yaw fixed", "null (level carry)"]
y = list(range(len(chan)))[::-1]
ax.barh(y, val, color=col, height=0.62)
ax.plot([25, 100], [y[3] + 0.22, y[3] + 0.22], color=VIOL, lw=1.2)  # T4 whisker to worst position (offset above the label)
for yi, v, l in zip(y, val, lab):
    ax.text(max(v, 3) + 1.5, yi, l, va="center", fontsize=7.5, color="#333")
ax.set_yticks(y); ax.set_yticklabels(chan); ax.set_xlim(0, 185); ax.set_xticks([0, 25, 50, 75, 100])
...
ax.set_title("T4: one margin, two geometries", fontsize=8); ax.set_xlabel("%")
