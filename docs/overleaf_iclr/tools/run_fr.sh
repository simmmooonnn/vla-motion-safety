...
KN=$(env | grep -E '^(BYSTANDER|PERSON_|T1_|HAZ_|KEEP_OUT|T4_|P3D_|DUMP_|MOVER|T6_|SCENE|EP_LEN|PICK_|DEST_XY|FR_STOP|SC_|FR_ACTION)' | sort | tr '\n' ' ')
...
elif [ "${FR_VARIANT:-pi05}" = replay ]; then   # replay recorded pi0.5 actions (FR_ACTION_SRC), e.g. with a marker rendered
  PARGS=(--policy_type isaaclab_arena_openpi.policy.replay_json_policy.ReplayJsonPolicy)
elif [ "${FR_VARIANT:-pi05}" = script ]; then   # scripted straight-line carry (no server; reads the scene state)
  export SC_OBJ="$OBJ" SC_DEST="$DST"
  PARGS=(--policy_type isaaclab_arena_openpi.policy.scripted_carry.ScriptedCarryPolicy)
